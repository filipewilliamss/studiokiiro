...
import { FilePlus, Printer, Hash, Calendar, Building2 } from "lucide-react";
...
const ServiceOrdersTab = () => {
  ...
  const [form, setForm] = useState({
    ...
    description: "", total_value: "", payment_terms: "", terms_conditions: defaultTerms, notes: "",
  });
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    if (error) toast.error("Erro ao criar OS");
      ...
      setOpen(false);
      setForm({ client_id: "", project_id: "", service_type: "Identidade Visual", description: "", total_value: "", payment_terms: "", terms_conditions: defaultTerms, notes: "" });
      fetchAll();
    }
    setLoading(false);
  };
      ...
      <div className="flex items-center justify-between">
        ...
        <Dialog open={open} onOpenChange={setOpen}>
          <DialogTrigger asChild>
            <Button className="gap-2"><FilePlus className="h-4 w-4" /> Nova OS</Button>
          </DialogTrigger>
          <DialogContent className="sm:max-w-lg max-h-[85vh] overflow-y-auto">
            <DialogHeader>
              <DialogTitle style={{ fontFamily: "var(--font-display)" }}>Nova Ordem de Serviço</DialogTitle>
            </DialogHeader>
            <form onSubmit={handleCreate} className="space-y-4 mt-2">
              ...
              <div className="flex justify-end gap-2 pt-2">
                ...
                <Button type="submit" disabled={loading || !form.client_id}>{loading ? "Criando..." : "Criar OS"}</Button>
              </div>
            </form>
          </DialogContent>
        </Dialog>
